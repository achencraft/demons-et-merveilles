...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <body>
        ...
        if(isset($_SESSION['username']))
        {
            ...
            <h3>Parties supprimées par les utilisateurs</h3>

            Liste des parties supprimées:
            ...
            $result = mysql_query('select count(*) as total from games where est_supprime = 1');
            $nbr_admin=mysql_fetch_assoc($result);
            $compteur = 0;
            if($nbr_admin['total'] != 0){

                //On recupere les identifiants, les pseudos et les emails des utilisateurs
                $req = mysql_query("select g.id,h.titre,h.nom_mj,c.debut,c.fin,t.emplacement,g.nbr_joueur_courant,h.nbr_joueur_max from games g join histoires h on g.histoire_id = h.id join creneaux c on g.creneau_id = c.id join tables t on g.table_id = t.id where  g.est_supprime = 1");
