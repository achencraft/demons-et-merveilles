...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <body>
        ...
        if(isset($_SESSION['username']))
        {
            ...
            <h3>Parties publiées</h3>

            Liste des parties publiées:
            ...
            $result = mysql_query('select count(*) as total from games g join histoires h on g.histoire_id = h.id where h.valide_par_admin = 1 and g.est_publie = 1');
            $nbr_admin=mysql_fetch_assoc($result);
            $compteur = 0;
            if($nbr_admin['total'] != 0){

                //On recupere les identifiants, les pseudos et les emails des utilisateurs
                $req = mysql_query("select g.id,h.titre,h.nom_mj,c.debut,c.fin,t.emplacement,g.nbr_joueur_courant,h.nbr_joueur_max from games g join histoires h on g.histoire_id = h.id join creneaux c on g.creneau_id = c.id join tables t on g.table_id = t.id where h.valide_par_admin = 1 and g.est_publie = 1");
                ...
                <?php
                    ...
                    <tr style="background-color:<?php if($compteur%2 == 0) {echo '#D8D8D8';} else { echo '#BCA9F5';} ?>;">
                    <td colspan="7">Il n'y a pas de parties publiées</td>
                </tr><?php
            }
            ?>
